...
export default class LineChart extends Chart {
  ...
  /**
   * Updates the chart content and style.
   *
   * @method _chartUpdate
   * @methodOf LineChart
   * @param {number} [duration = 700] Duration of the update animation in ms.
   * @private
   */
  _chartUpdate (duration = 700) {
    ...
    // Update axes
    this._dom.axes.update(this._attr, this._dom.scales, duration)
  ...
  /**
   * Creates the tooltip content based on the mouse position.
   *
   * @method _createTooltipContent
   * @methodOf LineChart
   * @param {number[]} mouse Array containing the mouse coordinates.
   * @returns {Object} Object representing the tooltip content.
   * @private
   */
  _createTooltipContent (mouse) {
    ...
    if (index === null) {
      for (let t in this.tt) {
        if (this.tt.hasOwnProperty(t)) { this.tt[t].remove() }
      }
      this.tt = null
      return
    } else {
  ...
  /**
     ...
     * @returns {(Object | undefined)} D3 selection of the marker if it could be added, undefined otherwise.
     */
